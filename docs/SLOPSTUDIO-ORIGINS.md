<!-- Moved here from the repository README when the fork went public. It documents the
SlopStudio / SpotForge editor that SlopDirector is built on: features, architecture, the
ffmpeg pipeline, the data model, env vars and the original web deployment. Some of it
predates the desktop/SQLite target and the agent harness. -->
...
> **The editor (NLE) is documented in [`docs/SLOPSTUDIO.md`](SLOPSTUDIO.md).** The rest
...
> - **[`docs/DEPENDENCIES.md`](DEPENDENCIES.md)** — every dependency (Node/pnpm, Postgres/SQLite,
>   ffmpeg, the Audio Studio Python/ML stack incl. the **torchcodec** requirement) **+ a
>   dedicated Windows setup section.**
> - **[`docs/AUDIO_STUDIO.md`](AUDIO_STUDIO.md)** — the DAW-style audio workspace (Demucs / Whisper).
> - **[`docs/TIMELINE.md`](TIMELINE.md)** — the unified multi-layer video+audio timeline (N layers, waveforms, tempo/beats, render parity).
> - **[`docs/PROJECT_BUNDLES.md`](PROJECT_BUNDLES.md)** — portable per-project folders (project.json + assets) and the storage model.
> - **[`docs/CHANGELOG.md`](CHANGELOG.md)** — recent changes (newest first).
> - **[`docs/DESKTOP.md`](DESKTOP.md)** — Electron / AppImage architecture.
...
> See also: [`../CLAUDE.md`](../CLAUDE.md) (the Claude Code harness brief) and
> [`../DEPLOY.md`](../DEPLOY.md) (generic single-box deploy runbook). This file is the
> full human-facing reference for the underlying editor.
...
> [`docs/DEPENDENCIES.md`](DEPENDENCIES.md#omarchy--arch-linux-setup-desktop-target-dev-no-postgres).
...
**Caddy** reverse proxy on a separate box terminates TLS for your domain,
