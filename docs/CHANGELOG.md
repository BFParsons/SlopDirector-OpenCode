...
- **The director's type.** Each directing style now carries a surveyed type system
  (`src/lib/typography/styleType.ts`; the *Type* section of each `guide/styles/<id>.md`,
  generated by `scripts/gen-style-type.ts`): what its titles, credits and captions actually
  use — Curtis's white Helvetica in sentence case, Anderson's Futura (Archer, Tilda as
  exceptions), Nolan's Gotham / Avenir / Helvetica Neue Black with Didot for Interstellar,
  Villeneuve's lightened Gotham and monumental tracked wordmarks, Alien's disjointed
  Helvetica Black and Gladiator's Trajan, The Social Network's Klavika, A24's one-serif-per-
  film, Nope's extended Helvetica, Morris's Baskerville (his own essay on the typeface of
  truth), Vox's Balto for Harris, Anton and Bebas Neue for MrBeast (bundled as the real
  faces), Tom Scott's Helvetica Black, Vsauce's DIN Next Rounded, Neistat's marker hand,
  McKinnon's brush face — with open-licence stand-ins for what cannot be bundled (31 new
  faces: Jost, Montserrat, Oswald, Bebas Neue, Anton, Cinzel, Playfair Display, EB Garamond,
  Cormorant Garamond, Libre Franklin, Libre Baskerville, Courier Prime, Zilla Slab, Michroma,
  Nunito, an expanded Archivo, Permanent Marker; licences in `public/fonts/LICENSES.md`),
  a case (sentence, capitals, spaced capitals, lower-case — tracking done in the text since
  drawtext has none), a colour and field, an entrance, and the roles the style puts on the
  frame (title, card, intertitle, lower-third, caption, callout, citation, date, quote, label,
  credit) with the roles it never uses; Herzog, Wiseman, Jennings, Malick and Cunningham are
  no-text styles. `add_text_overlay {role}` resolves through the brief's style;
  `list_typography` returns the style's system; `check_text` (and `verify_export`) flag text
  on a no-text style, a face outside the family, capitals on a sentence-case style, a role the
  style never uses. Where the survey found no documentation (Burns, Veritasium, Rober, Zhou,
  Jennings, Guest) the note says so. Guide 12.10; RULES 37.
- **Riefenstahl removed from the directing styles** at the user's request; the political and
  propaganda category keeps Capra, Eisenstein, Schwartz and the Lincoln Project.
...
- **Directing styles.** The interview asks "Whose eye?" after the genre and offers directing
  styles fashioned after real filmmakers and houses — 42 of them across nine categories
  ...
  Capra, Eisenstein, Schwartz, the Lincoln Project; trailers: Woollen, Buddha
...
- **Pre-production: interview → brief → plan → approval → fan-out.** Both real jobs started
  ...
  The plan is shown in the terminal, not in a file: `plan_document` defaults to box-drawn
  tables (`?view=table&width=110`) — beats, then a two-column AV script with one row per
  shot (# / at / len / sound / VIDEO = picture, source, card / AUDIO = the narration and
  bite lines over it), clips to find, AI shots — followed by music, narration, risks and the
  check; the agent pastes it verbatim in a code block. `format: "list"` is the same as an
  indented list (`?view=cli`); markdown (`format: "markdown"`, also written to
  `plan-v<N>.md`) stays for files.
