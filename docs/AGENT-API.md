...
- `GET/PUT /api/projects/:id/brief` — the interview's outcome (`src/lib/validation/brief.ts`: deliverable standalone|scene, length, aspect; scripted + genre; sources; premise; tone; audience; narration/music/text; must-include/avoid).
  `production.style.id` names a directing style (`src/lib/styles`, prose in `guide/styles/<id>.md`); `check_plan` reviews its editorial defaults. Research-backed craft records include sources and material prerequisites.
...
- Plans optionally carry `styleTreatment`: reference, mechanism, materialPlan, rhythm, sound, typography, exceptions and evaluation. New plans for researched references should fill it; checks warn on omission. All plan document formats and production tasks include it. No database migration is required (the plan remains JSON).
