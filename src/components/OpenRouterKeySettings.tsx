...
export function OpenRouterKeySettings({ initialHint }: { initialHint: string | null }) {
  ...
  async function save() {
    ...
    setSaved(false);
    try {
      const res = await api<{ hint: string }>("/api/account/openrouter-key", {
        method: "POST",
  ...
  async function remove() {
    ...
    setSaved(false);
    try {
      await api("/api/account/openrouter-key", { method: "DELETE" });
      setHint(null);
    } catch (e) {
      setError((e as Error).message);
    } finally {
    ...
    <Card className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-xs text-[var(--color-muted)]">
          Use your own OpenRouter key so generation is billed to your account.
          Get one at{" "}
          <a
