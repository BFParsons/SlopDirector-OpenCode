...
37. Text is a shot (ch.12): it anchors to the title-safe area (never outside action-safe), one family per piece, sizes by hierarchy (title 8–10 %, card 4–6, lower-third 3.5–4.5, callout 3–3.5 of the frame height), lines under 44 characters (26 vertical), held for the reading time, one entrance vocabulary, one text at a time. `add_text_overlay` with a preset does this by default; `check_text` before the render and `verify_export` after it say when it is not.
