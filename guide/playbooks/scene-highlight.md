...
5. `check_soundtrack` — no clashes; with narration over a bed, `balance_music` (gapLu 6) — then continue.
6. `render_draft`; `check_mix_levels`; `pacing_report` (compare against the genre norm); `check_cuts`; `detect_scenes` on the draft should find one cut per boundary you made.
