...
4. Sound: the speaker's shots keep their sound (`muted:false`), everything else is muted; music, if any, via `set_music` then `balance_music` (gapLu 6–8: captions-first viewers still need the voice) — never a guessed level; `check_soundtrack`. Reframe, don't crop (rule 22): `add_segment` then `update_segments` with `transform` (scale/pan keyframes) so the subject sits in the middle third; replace wides with closer coverage or drop them. Verify every shot with `get_frame`.
...
6. `render_draft`; `check_mix_levels`; `verify_export` target `social` (loudness −14 LUFS); `check_cuts`; `get_contact_sheet` of the draft to confirm framing and caption legibility.
