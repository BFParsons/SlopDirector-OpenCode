...
> Companion guide for a Linux video editing application with an MCP server/harness. Written for human editors and for agents operating the app. Software-specific sections are left as placeholders (`[TOOL: ... — TBD]`) to be filled in once the tool surface is finalized.
...
*To be written. Software-specific; deferred until the data model and MCP tool surface are finalized.*
...
Software-specific tool mappings are marked `[TOOL: ...]` and left blank for now.
...
*Mechanical check:* for dialogue coverage, tag each setup with the side of the line it was shot from. Flag any adjacent pair of clips with different sides and no neutral shot between them. `[TOOL: metadata tagging / adjacency query — TBD]`
...
*Mechanical check:* compare motion vectors or the position of a tracked feature across the last frame of the outgoing shot and the first frame of the incoming one. Large discontinuities in position or velocity for the same subject indicate a mismatch. `[TOOL: frame grab / motion analysis — TBD]`
...
*Mechanical check:* compute shot count, ASL, median, standard deviation, and a histogram of durations for any range. Compare across versions. `[TOOL: timeline query / statistics — TBD]`
...
*Mechanical check:* for each dialogue cut, report the offset between the video cut point and the audio cut point. Flag runs of straight cuts. `[TOOL: timeline query — TBD]`
...
*Mechanical check:* transcript-based detection of filler words and pauses over a threshold; report candidates with timecodes. Do not auto-remove; propose. `[TOOL: transcript query — TBD]`
...
*Workflow:* duplicate a range, swap the shot preceding the reaction, render both, compare. `[TOOL: range duplicate / render / compare — TBD]`
...
*Mechanical check:* for a dialogue scene, compute the position of the dominant face or subject at the last frame of each outgoing shot and the first frame of each incoming shot. Flag cuts where the distance is large and no motion motivates it. `[TOOL: frame grab / subject tracking — TBD]`
...
*Mechanical check:* extract the beat grid of a music cue; report the offset in frames of every cut in the range from the nearest beat. Flag cuts more than 2 frames off-grid. `[TOOL: audio beat detection / timeline query — TBD]`
...
*Workflow:* transcript search returns timecoded passages; marking creates markers or ratings on the source clip. `[TOOL: transcript query / marker create — TBD]`
...
*Mechanical check:* per-version report of total runtime, per-scene runtime, and per-scene shot statistics; diff against the previous version. `[TOOL: version compare / statistics — TBD]`
...
**Rule: Before lock, run every mechanical check once more.** Gaps, flash frames, sync, missing media, offline clips. `[TOOL: automated checks — TBD]`
...
**Rule: Every post-lock change produces a change list.** Which frames were added, removed, or moved, relative to the locked reference. `[TOOL: timeline diff / change list — TBD]`
...
**Rule: Use a consistent marker color or tag for note status: open, addressed, rejected.** `[TOOL: marker create / update — TBD]`
...
**Rule: Check for collateral damage.** A note addressed at 00:13:00 can break sync at 00:13:30 or shift a music cue at 00:15:00. `[TOOL: timeline diff / automated checks — TBD]`
...
*Workflow:* render both versions of a range, play back-to-back or side by side, record the preference and the reason. `[TOOL: range render / compare — TBD]`
...
**Rule: Keep a version log with runtime, what changed, and who requested it.** The log is the edit's memory. `[TOOL: version create / list — TBD]`
...
**Rule: Review exports are low resolution, fast to make, and small enough to send.** They are not deliverables. `[TOOL: render preset / burn-in — TBD]`
...
*Mechanical check:* list all clips whose media type or naming doesn't match their track's purpose tag. `[TOOL: timeline query / track metadata — TBD]`
...
*Mechanical check:* find every gap on dialogue tracks longer than 1 frame; report and optionally fill with the location's room tone clip. `[TOOL: gap detection / fill — TBD]`
...
*Mechanical check:* for each clip with linked audio, compare the offset between video and audio at head and tail; flag drift. For dialogue clips, optionally compare waveform transients against a visual mouth-motion estimate. `[TOOL: sync analysis — TBD]`
...
**Rule: Track every temp cue: what it is, where it came from, and whether it's clearable.** A temp track that becomes essential and can't be licensed is a disaster discovered too late. `[TOOL: clip metadata / music log — TBD]`
...
*Mechanical check:* detect the beat grid of a music clip; report each cut in the range with its frame offset from the nearest beat and nearest phrase boundary. `[TOOL: beat detection / timeline query — TBD]`
...
*Mechanical check:* measure integrated LUFS, short-term LUFS range, and true peak for the timeline or a range; compare against a named platform target. `[TOOL: loudness measurement — TBD]`
...
*Workflow:* export stems by track group; export AAF with handles; export picture reference with BITC and 2-pop; generate track sheet from track metadata. `[TOOL: stem export / AAF export / reference render — TBD]`
...
*Mechanical check:* for each clip, report clipped highlight and shadow percentage, RGB channel offsets in shadows and highlights, and skin tone hue deviation where a face is detected. `[TOOL: scope analysis — TBD]`
...
*Workflow:* group clips by camera and setup metadata; apply the hero correction to the group; report residual scope differences per clip. `[TOOL: clip grouping / correction apply / scope diff — TBD]`
...
*Workflow:* generate lower thirds from a template and a list of names and timecodes; verify that none overlaps a cut. `[TOOL: template instantiate / timeline query — TBD]`
...
*Mechanical check:* validate caption files for line length, duration, reading speed, overlap, and proximity to shot changes. `[TOOL: caption validation — TBD]`
...
*Workflow:* render plates for a list of marked shots with specified handles and format; generate the shot list; on return, match by shot ID and replace. `[TOOL: plate render / shot list / replace by ID — TBD]`
...
**Rule: On Linux, verify that the codec/container combination encodes correctly with the app's FFmpeg build, and that the destination decodes it.** Not every combination is supported everywhere. `[TOOL: encoder capability query — TBD]`
...
**Rule: Build a render preset per destination, name it for the destination and the spec version, and don't edit it in place.** New spec, new preset. `[TOOL: render preset create — TBD]`
...
**Rule: Log every render: source timeline and version, preset, output path, duration, checksum.** `[TOOL: render queue / render log — TBD]`
...
*Mechanical check:* probe the exported file; compare every technical parameter against the preset and the spec; measure loudness; detect black frames, frozen frames, silent audio, and clipped audio; compare frame grabs against the timeline at sampled timecodes. `[TOOL: export verification — TBD]`
...
**Rule: Every AAF and FCPXML export is tested by reimporting it into a fresh project and comparing against the original.** If it doesn't round-trip, it won't survive the trip to another tool. `[TOOL: export / import / timeline diff — TBD]`
...
**Rule: After any relink, run a full mechanical check: offline clips, duration mismatches, frame rate mismatches, audio channel mismatches.** `[TOOL: relink / verification — TBD]`
...
**Rule: Search transcripts by idea, not by keyword.** The line that makes the film may not contain the word you're looking for. Read the surrounding context of every hit. `[TOOL: transcript semantic search — TBD]`
...
**Rule: Reframing is a creative pass and gets watched shot by shot, not batch-applied.** Auto-reframing tools are a starting point; a human or an agent with frame-grab verification checks every result. `[TOOL: reframe / frame grab — TBD]`
...
**Rule: Keep a manifest linking every short to its source timecode in the long-form master.** For rights, for corrections, for finding the next one. `[TOOL: range export / manifest — TBD]`
...
**Rule: Rate-limit and log everything.** An agent making a thousand edits per minute is either doing a batch job it was asked to do or malfunctioning; the log tells you which. `[TOOL: operation log / rate limit — TBD]`
...
*End of Part VIII. Appendices A–C (MCP tool reference, shortcuts and CLI, glossary) are software-specific and deferred with Parts I–III.*  
